...
from .modeling_llama_rope_pp import LlamaForCausalLM
...
class RoPEPPCausalLM(HuggingFaceBaseModel):
    ...
    def _load_model(self, path: str, kwargs: dict, peft_path: Optional[str] = None, peft_kwargs: dict = dict()):
        ...
        self.config.__setattr__("rope_config", self.rope_config)

        if 'scaling_factor' in self.rope_config:
            self.config.rope_theta = self.config.rope_theta * self.rope_config['scaling_factor']
