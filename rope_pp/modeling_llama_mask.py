...
class LlamaAttention(nn.Module):
    ...
    def apply_rotary_pos_emb(self, q, k, cos, sin, unsqueeze_dim=1):
        ...
        if self.config.rope_config['imag'] and self.config.rope_config['imag_mode'] == 'imagh':
            ...
            if self.config.mask_config is not None and self.config.mask_config['noise_std'] > 0:
                noise_std = self.config.mask_config['noise_std']
                if self.config.mask_config['noise_target'] in ['real', 'both']:
                    noise = torch.randn_like(q_rope, device=q_rope.device, dtype=q_rope.dtype) * noise_std
                    q_rope += noise
                if self.config.mask_config['noise_target'] in ['imag', 'both']:
                    noise = torch.randn_like(q_imag, device=q_imag.device, dtype=q_imag.dtype) * noise_std
                    q_imag += noise
