...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, config: LlamaConfig, device=None):
        ...
        if 'yarn' in config.rope_config and config.rope_config['yarn']:
            r = 4096 / 2 / math.pi * inv_freq
            gamma = torch.clip((r - 1) / (32 - 1), max=1., min=0.)
            inv_freq = (1 - gamma) * inv_freq / config.rope_config['factor'] + gamma * inv_freq
            scaling = scaling * (0.1 * math.log(config.rope_config['factor']) + 1)
        elif 'factor' in config.rope_config:
            inv_freq = inv_freq / config.rope_config['factor']
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        if self.config.rope_config['imag']:
            ...
            # elif self.config.rope_config['imag_mode'] == 'imagm':
            #     num_q_heads, num_kv_heads = config.num_attention_heads, config.num_key_value_heads // 2
            #     self.num_key_value_groups = self.num_key_value_groups * 2
            elif self.config.rope_config['imag_mode'] in ['imagh', 'imago', 'imagx']:
                num_q_heads, num_kv_heads = config.num_attention_heads, config.num_key_value_heads
                num_attention_heads = config.num_attention_heads
            else:
    ...
    def apply_rotary_pos_emb(self, q, k, cos, sin, unsqueeze_dim=1):
        ...
        if self.config.rope_config['imag'] and self.config.rope_config['imag_mode'] == 'imagh':
        ...
        elif self.config.rope_config['imag'] and self.config.rope_config['imag_mode'] == 'imagx':
            batch, num_attention_heads, slen, head_dim = q.shape
            q[:, 0::2, ...] = q[:, 0::2, ...] * cos + rotate_half(q[:, 0::2, ...]) * sin
            q[:, 1::2, ...] = q[:, 1::2, ...] * sin - rotate_half(q[:, 1::2, ...]) * cos
            k_rope = k * cos + rotate_half(k) * sin
            q_rope = q
        else:
            q_rope = q * cos + rotate_half(q) * sin
            ...
            if q_imag is not None:
                ...
                q_rope = torch.stack([q_rope, q_imag], dim=2).reshape(batch, num_attention_heads * 2, slen, head_dim)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #     print(f'{self.layer_idx = }, {query_states.shape = }, {key_states.shape = }, {value_states.shape = }, ')
